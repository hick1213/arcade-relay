...
Codex invocation: `$forge-concept`.
...
| `state/stage.txt` | Read | 若为 `concept` 之后则警告「Phase 1 已完成。重新执行将覆盖 design/ 下的内容」，并用 Codex 对话询问 确认是否继续 |
...
- scriptPath: `.codex/workflows/concept-design.js`
...
2. **产出物路径**: 上述 5 个文件
3. **Key image 候选**: 将返回值记载的候选图像**用 Codex 文件附件（display: render）显示**。附注: 此处批准的 1 张将成为 `design/art-bible.json` 风格锁定的基准
...
展示的同时发送 **Codex 通知**（例: 「ArcadeRelay: Checkpoint A（策划设计批准）已准备就绪」）。

### reviewMode = solo 时

**不停止**。仅进行上述展示与 Codex 通知，key image 视为采用候选第 1 位，直接进入 Phase 5。

## Phase 4: 批准循环（仅 full / lean）

用 Codex 对话询问 请求判断。选项:
...
- **修改指示** → 按以下步骤（**重新展示最多 1 次**）:
  ...
  3. 用修改后的产出物重新执行 Phase 3 的展示，再次 Codex 对话询问
  4. 第 2 次仍未获批准时: 将剩余问题追加写入 `state/checkpoint-a-feedback.md`，用 Codex 对话询问 确认「带着未解决事项批准 / 在此中断（stage 保持不变）」二选一。中断则停止
