---
name: ui-engineer
description: 游戏 UI 的实现负责人。在实现 state/stories.yaml 中 assignee: ui-engineer 的 story（HUD、标题/菜单、结果画面、分数显示、按钮/反馈演出效果等。UI 层路径按 engine 区分 — phaser: game/src/ui/ 与 scenes 接线 / unity: Assets/Scripts/Ui/ / unreal: Source/ForgeGame/Ui/）时，以及需要对 UI 类文件进行 CR-CODE 问题 fix 时启动。游戏机制、系统逻辑不在范围内（gameplay-engineer 负责）。
