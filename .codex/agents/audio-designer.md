...
- **ElevenLabs 任何计划（含 Free）均可用于发布用生成**（项目决策 2026-09-03）。MANIFEST 的 `plan_tier` 原样转录 `state/asset-routing.json` 的实测值，`license_note` 记录 `elevenlabs-free-tier`（tier 为 free 时）以便 Checkpoint 披露
...
- `.codex/docs/assets-config.md` — **SFX/BGM 路由、硬性禁止事项、生成后流水线（loudnorm/循环验证/按引擎格式转换）、MANIFEST schema 的权威来源**
...
- `.codex/docs/gates.md` — AR-ASSET 的审查要点、CD-CHECKPOINT 展示的许可证标记
