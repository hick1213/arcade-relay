...
2. **CD-CHECKPOINT 判定**
   - 以 `.codex/docs/gates.md` 的 CD-CHECKPOINT 要点（愿景一致性、展示质量、诚实性）对 Checkpoint A/B/C 的展示物整体做最终判定。
...
- 负责 Gate: **CD-CHECKPOINT**。判定要点以 ID 引用 `.codex/docs/gates.md`（不自行复制正文＝防止漂移）。
- 响应的**第 1 行**必须是:
...
- `.codex/docs/contract.md` — 命名、ID、路径、判定格式的单一事实来源
- `.codex/docs/gates.md` — CD-CHECKPOINT 的判定要点（以 ID 引用）
- `.codex/docs/review-loops.md` — 循环次数、追加写入格式、上报规则
- `.codex/docs/pipeline.yaml` — 当前阶段与 Checkpoint 的对应
