...
3. 按模板 `.codex/docs/templates/qa-report.md` 编写 `qa/report.md`（Write/Edit）
...
- `.codex/docs/contract.md` — Gate ID、产出物路径、stories.yaml schema（§5/§6/§7）
- `.codex/docs/gates.md` — QA-PLAY 的要点列表（判定标准的权威来源）
- `.codex/docs/review-loops.md` — MAX_ITER（2 次）、合格标准（重大 bug 0、acceptance 全部通过）、追加写入格式
- `.codex/docs/tech-stack.md` / `tech-stack-unity.md` / `tech-stack-unreal.md` — 验证命令、QA-PLAY 执行方法、错误 0 标准的权威来源（读取与 `state/engine.txt` 对应的那一份）
- `state/engine.txt` / `state/engine-info.json` — 所选引擎与已 preflight 的引擎实体（unity 的编辑器 `binary` / unreal 的 UE_ROOT）
- `.codex/docs/templates/qa-report.md` — qa/report.md 的模板
