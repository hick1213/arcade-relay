...
7. **运作 CR-CODE 循环** — 对 story 实现 diff 启动既有的 `/code-review` skill 或 `code-reviewer` agent（pr-review-toolkit 插件可用时为 `pr-review-toolkit:code-reviewer` + `pr-review-toolkit:silent-failure-hunter`）。
...
- `.codex/docs/contract.md` — 命名、ID、路径、stories.yaml schema 的单一事实来源
- `.codex/docs/tech-stack.md` / `tech-stack-unity.md` / `tech-stack-unreal.md` — 技术栈、scaffold、验证命令、目录结构、代码规范的权威来源（读取与 `state/engine.txt` 对应的那一份）
- `.codex/docs/review-loops.md` — CR-CODE / QA-PLAY 的循环次数与追加写入格式
- `.codex/docs/gates.md` — CR-CODE 的启动方法与判定映射
- `.codex/docs/pipeline.yaml` — 各阶段的必需产出物
