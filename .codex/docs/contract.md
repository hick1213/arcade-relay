...
## 2. agent 名（`.codex/agents/<name>.toml`、仅此 11 个）

Codex loads the TOML entry point. Each entry delegates to the adjacent Markdown
role contract with the same name; the Markdown file is not a Claude tool
definition.
...
CR-CODE reviewer: `code-reviewer` (Codex project subagent)

代码审查不新建 agent，使用环境内可用的现有审查 agent（默认 `code-reviewer`；装有 pr-review-toolkit 插件的环境可用 `pr-review-toolkit:code-reviewer` / `pr-review-toolkit:silent-failure-hunter`。silent-failure 检查点已并入同一次评审提示词 — gates.md CR-CODE 节）。

## 3. skill 名（`.agents/skills/<name>/SKILL.md`、仅此 6 个）

公开名称为 ArcadeRelay，但为与现有运用兼容，命令命名空间保持为 `forge`。

`forge` `forge-brainstorm` `forge-concept` `forge-prototype` `forge-build` `forge-status`

## 4. Workflow 脚本（`.codex/workflows/<name>.js`、仅此 3 个）
...
review→revise 的最大迭代数与合格标准定义在 `.codex/docs/review-loops.md`。
...
环境变量: `FAL_KEY` `ELEVENLABS_API_KEY` `RETRO_DIFFUSION_API_KEY`（可选: `IDEOGRAM_API_KEY` `OPENAI_API_KEY` `OPENAI_BASE_URL`（OpenAI 兼容中转的基址，默认 `https://api.openai.com/v1`） `MESHY_API_KEY` `TRIPO_API_KEY`。预算初始值: `ASSET_BUDGET_USD` → `state/budget.txt`）
2D 图像（image_sprite / image_background）: **Primary 在 fal.ai 与 OpenAI `gpt-image-2` 之间二选一**，`FAL_KEY` 与 `OPENAI_API_KEY` 只需其一有效；两者都有效时由 preflight 一次性询问并固定写入 `state/asset-routing.json`（之后禁止重新判定）。背景去除与 LoRA 训练无 OpenAI 替代，仅 `OPENAI_API_KEY` 有效时 `bg_removal` 降为本地 rembg — 详见 assets-config.md「图像 Primary 二选一」
...
|---|---|---|---|---|---|
| `phaser` | 2D | `.codex/docs/tech-stack.md` | `game/package.json` | `rules/gameplay-code.md` + `rules/ui-code.md` | `game/src/**` |
| `unity` | 3D | `.codex/docs/tech-stack-unity.md` | `game/ProjectSettings/ProjectVersion.txt` | `rules/unity-code.md` | `game/Assets/Scripts/**`（.cs） |
| `unreal` | 3D | `.codex/docs/tech-stack-unreal.md` | `game/ForgeGame.uproject` | `rules/unreal-code.md` | `game/Source/**`（.cpp/.h） |
