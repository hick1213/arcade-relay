#!/bin/bash
# ArcadeRelay hook: Codex PostToolUse advisory check
...
case "$FILE_PATH" in
  ...
  */game/Assets/*.cs|game/Assets/*.cs)
    echo "[ArcadeRelay] 修改了 game/Assets/ 的 C#。修改后请用 .codex/docs/tech-stack-unity.md 的「验证命令」（batchmode 编译验证）确认 exit 0。"
    ;;
  */game/Source/*.cpp|game/Source/*.cpp|*/game/Source/*.h|game/Source/*.h)
    echo "[ArcadeRelay] 修改了 game/Source/ 的 C++。修改后请用 .codex/docs/tech-stack-unreal.md 的「验证命令」（UnrealBuildTool 构建）确认 exit 0。"
    ;;
esac
