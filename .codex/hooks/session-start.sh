...
# --- 解析项目根目录（优先 Codex/Claude 环境变量，没有则从 git/脚本反推） ---
...
PIPELINE="$ROOT/.codex/docs/pipeline.yaml"
...
if [ ! -f "$PIPELINE" ]; then
  ...
  echo "[ArcadeRelay] 找不到 pipeline.yaml（.codex/docs/pipeline.yaml）。harness 已损坏。请用 /forge-status 确认。"
  exit 0
fi
