...
生成与放置规则的权威来源是 `.codex/docs/assets-config.md`。此处是放置时生效的检查清单。
...
- **Do**: 每次添加、替换、修饰资产时，向 MANIFEST（该引擎的权威来源路径 — contract §6）追加写入1行。必需字段: `file` `provider` `model` `prompt` `cost_usd` `plan_tier` `sha256` `license` `generated_at`。`seed` `style_codes` 在提供方支持时必需（fal/Ideogram 路径）；不支持的提供方（如 OpenAI gpt-image-2 — 无 seed、无 style_codes）记为 `"seed": null, "style_codes": []`，不得省略键本身。若存在提供方特有的标注条款（Ideogram 标注条款 / Hunyuan3D Territory / ElevenLabs Studio Games 等），`license_note` 也为必需（assets-config.md「Provenance」）。占位资产还需加上 `"must_replace": true`
...
- **Don't**: 不要把许可不明、非商用（`bria-rmbg` 输出、audiocraft 输出等。ElevenLabs Free 按项目决策记为 `commercial-ok` + `license_note: elevenlabs-free-tier`）的资产记录为 `license: commercial-ok`
...
Primary 为 `openai:gpt-image-2` 时（无 seed/style_codes；`cost_usd` 按分辨率档位定价 — assets-config.md「gpt-image-2 API 要点」，非精确 token 用量核算时须加 `cost_estimated:true`）:

```json
{"file":"assets/sprites/sprite-hero-idle.png","provider":"openai:gpt-image-2","model":"gpt-image-2","prompt":"<style_block> + hero idle pose","seed":null,"style_codes":[],"cost_usd":0.03,"cost_estimated":true,"plan_tier":"unknown","sha256":"e3b0c442...","license":"commercial-ok","generated_at":"2026-07-03T12:00:00Z"}
```
