// .codex/workflows/full-build.js — Phase 3（由 /forge-build 启动，contract.md §4）
...
// 规范的权威来源: .codex/docs/contract.md / review-loops.md / gates.md / tech-stack.md / assets-config.md
...
async function implementStoryWithReview(story, phaseName) {
  ...
  for (let iter = 1; iter <= 1; iter++) {
    ...
    const review = await agentR(reviewPrompt, { label: 'cr-' + sid + '-' + iter, phase: phaseName, agentType: 'code-reviewer', schema: CODE_REVIEW_SCHEMA });
    if (!review) {
  ...
  if (!approved) {
    // 状态文件＝事实（AGENTS.md 绝对规范）: 即使在评审 agent 失败等导致 fix agent 的 status 更新
...
const assetGenThunks = [
  laneSafe('AssetGen(images) 轨道', () => assetBatchLoop(
    ...
    '图像 Primary 遵循 state/asset-routing.json 的 routes.image_sprite / image_background（fal.ai 或 openai:gpt-image-2 二选一，生成中禁止改选）。' +
    '严格遵守风格一致性协议: 所有提示词机械地前置 art-bible.json 的 style_block；fal/Ideogram 路径记录 seed，gpt-image-2 路径无 seed/style_codes（MANIFEST 记 seed:null, style_codes:[]），响应为 b64_json 直接落盘（中转返回 url 则立即下载）。gpt-image-2 基址一律用 ${OPENAI_BASE_URL:-https://api.openai.com/v1}（= asset-routing.json 的 checks.openai.base_url。禁止硬编码官方域名）。hero 共用 character_reference。' +
