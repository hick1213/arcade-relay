...
// 命名、ID、路径全部遵循 .codex/docs/contract.md。循环规范见 .codex/docs/review-loops.md。
...
export const meta = {
  ...
  phases: [
    ...
    { title: 'Build', detail: '将 phase:prototype story 按 assignee lane（gameplay/ui）并行实现（lane 内顺序执行、lane 中不做引擎验证），每个 story 通过 CR-CODE 评审循环（MAX 1、单一 reviewer）。lane 合流后串行执行批量验证' },
    { title: 'AssetGen', detail: 'art-director 生成核心循环必需的图像（engine=unity/unreal 时还包括 3D 模型 MDL/ANM），audio-designer 生成核心 SFX，并通过 AR-ASSET 循环（MAX 2 + fallback 1）' },
    ...
    { title: 'QA', detail: 'qa-lead 执行 QA-PLAY（实际启动、实际操作、证据），重大 bug 进入修复循环（MAX 1）' },
...
// ---------------------------------------------------------------------------
const DOCS = {
  contract: '.codex/docs/contract.md',
  gates: '.codex/docs/gates.md',
  reviewLoops: '.codex/docs/review-loops.md',
  techStack: '.codex/docs/tech-stack.md', // 按 engine 区分 — 下方 EP 确定后替换为 engine 对应值
  assetsConfig: '.codex/docs/assets-config.md',
};
const ART = {
...
const ENGINE_PROFILES = {
  phaser: {
    ...
    codeRulesFile: '.codex/rules/gameplay-code.md',
  ...
  unity: {
    ...
    codeRulesFile: '.codex/rules/unity-code.md',
  ...
  unreal: {
    ...
    codeRulesFile: '.codex/rules/unreal-code.md',
...
const setupPrompt = [
  ...
  '5. 更新 ' + STATE.active + '（日期时间使用 `date -u +%Y-%m-%dT%H:%M:%SZ` 的执行输出 — 禁止凭推测填写），限定路径 add 后提交: `git add game docs state design && git commit -m "phase2: scaffold + stories"`（禁止 `git add -A` — 不要卷入 .codex/ 等非作业对象的变更）。',
  '',
...

{
  ...
  if (problems.length > 0 && setup) {
    ...
    setup = await agentR(
      ...
      { label: 'setup-fix-required-stories', phase: 'Setup', agentType: 'tech-director', effort: 'medium', schema: SETUP_SCHEMA }
    );
...
async function buildStoryLane(laneStories) {
  for (const story of laneStories) {
    ...
    const loopResult = await reviewLoop({
      gateId: 'CR-CODE',
      artifactName: sid,
      maxIter: 1, // review-loops.md: CR-CODE MAX_ITER 1（单一 reviewer 以节省 token）
      ...
      produce: async function () {
        const r = await agentR(
          ...
          { label: 'implement-' + sid, phase: 'Build', agentType: story.assignee, effort: 'medium', schema: COMMIT_RESULT_SCHEMA }
        );
        if (r && r.commitHash) {
      ...
      review: async function (iteration) {
        // CR-CODE 单一 reviewer（code-reviewer。为节省 token 不再并行 silent-failure-hunter — gates.md CR-CODE 节）
        const reviewCommon = [
        ...
        const result = await agentR(
          reviewCommon.concat([
            '',
            '要点: 除常规代码评审外，还要确认是否违反 ' + EP.codeRulesFile + '（若不存在则为 ' + DOCS.techStack + ' 的代码规范）— 尤其是混入魔法数字与不依赖 delta-time 的实现。也要重点排查 silent failure（被静默吞掉的异常、空 catch、隐藏失败的 fallback、无视错误、失败时伪装成功的返回值）。',
      ...
      revise: async function (findings, iteration) {
        const r = await agentR(
          [
            '你是 ArcadeRelay 的实现 engineer。修复 CR-CODE 评审问题（code-reviewer 指出的 findings）。',
            storyHeader,
            '',
            '问题列表:',
          ...
          { label: 'fix-' + sid + '-iter' + iteration, phase: 'Build', agentType: story.assignee, effort: 'medium', schema: COMMIT_RESULT_SCHEMA }
        );
        if (r && r.commitHash) {
          lastCommitHash = r.commitHash;
          return r;
...
// AR-ASSET 循环: 每个资产 MAX 2 + 切换 fallback 提供商后再 1 次（review-loops.md）
const GEN_SCHEMA = {
  type: 'object',
...
async function assetBatchLoop(cfg) {
  ...
  // 2次不合格 → 切换到 fallback 提供商后再 1 次（review-loops.md）
  if (failed && failed.length > 0) {
    log('[AR-ASSET] ' + cfg.batchName + ': 2次不合格 → 切换 fallback 提供商（' + failed.length + ' 个）');
    ...
    if (fb !== null) {
      const finalReview = await reviewBatch(3, '（切换 fallback 提供商后的最终判定。这是最后一次迭代）');
...
async function generateImages() {
  return assetBatchLoop({
    ...
    generatePrompt: [
      ...
      assetCommonRules,
      '图像 Primary 遵循 ' + STATE.assetRouting + ' 的 routes.image_sprite / image_background（fal.ai 或 openai:gpt-image-2 二选一，生成中禁止改选 — assets-config.md「图像 Primary 二选一」）。',
      '风格一致性: 将 ' + ART.artBibleJson + ' 的 style_block 机械地前置于全部提示词；fal/Ideogram 路径记录 seed，gpt-image-2 路径无 seed/style_codes（MANIFEST 记 seed:null, style_codes:[]）。hero 系共用 character_reference（gpt-image-2 经 /v1/images/edits 传入参考图）。',
      '全数执行生成后流水线（' + DOCS.assetsConfig + ' 所载）: fal/Ideogram 立即下载（URL 会失效）/ gpt-image-2 响应为 b64_json 直接落盘（中转返回 url 则立即下载；基址一律用 ${OPENAI_BASE_URL:-https://api.openai.com/v1}，禁止硬编码官方域名）→ Alpha 验证（禁止发布白背景 PNG）→ 必要时去背景 → 裁剪。',
    ...
    fallbackPrompt: function (failed) {
      return [
        '你是 ArcadeRelay 的 art-director。对 2次不合格的图像，切换到 ' + DOCS.assetsConfig + ' 路由表的 Fallback 提供商仅重新生成 1 次。',
        '对象:',
...
async function generateAudio() {
  return assetBatchLoop({
    ...
    generatePrompt: [
      ...
      assetCommonRules,
      '路由: ElevenLabs SFX v2 的 REST 直连（禁止官方 MCP、明示 duration_seconds、循环素材 loop:true）。任何计划（含 Free）均可做发布用生成，MANIFEST 转录 plan_tier。',
    ...
    fallbackPrompt: function (failed) {
      return [
        '你是 ArcadeRelay 的 audio-designer。对 2次不合格的 SFX，切换到 ' + DOCS.assetsConfig + ' 的本地降级路由（jsfxr。公有领域、确定性、可发布）仅生成 1 次。',
        '对象:',
        failed.map(function (f) { return '- ' + f.file + ': ' + f.reason; }).join('\n'),
        assetCommonRules,
      ].join('\n');
...
async function generateModels() {
  return assetBatchLoop({
    ...
    fallbackPrompt: function (failed) {
      return [
        '你是 ArcadeRelay 的 art-director。对 2次不合格的 3D 资产，切换到 ' + STATE.assetRouting + ' 的 fallbacks（直连 API 或本地降级）仅重新生成 1 次（参见 ' + DOCS.assetsConfig + ' 的 3D 路由表。降级生成为 must_replace: true）。',
        '对象:',
        failed.map(function (f) { return '- ' + f.file + ': ' + f.reason; }).join('\n'),
        assetCommonRules,
      ].join('\n');
...
const integrate = await agentR(
  [
    ...
    '8. 更新 ' + STATE.active + '（日期时间使用 `date -u +%Y-%m-%dT%H:%M:%SZ` 的执行输出 — 禁止凭推测填写），限定路径 add 后提交: `git add game docs state design && git commit -m "phase2: integrate assets"`（禁止 `git add -A` — 不要卷入 .codex/ 等非作业对象的变更）。',
    IDEMPOTENT_RULE,
    '结构化返回: ok / degradations / summary（包含已集成的资产键列表与缺失列表）。',
  ].join('\n'),
  { label: 'integrate-assets', phase: 'Integrate', agentType: 'gameplay-engineer', effort: 'medium', schema: INTEGRATE_SCHEMA }
);
if (integrate === null) {
...
let qaResult = null;
const QA_MAX = 1; // review-loops.md: QA-PLAY MAX_ITER 1（不做二次复核以节省 token）
for (let round = 1; round <= QA_MAX; round++) {
  qaResult = await agentR(
    ...
    { label: 'qa-play-round' + round, phase: 'QA', agentType: 'qa-lead', effort: 'medium', schema: qaSchema }
  );
  ...
  {
    // MAX_ITER 1: 仅 1 次 QA-PLAY，非 APPROVE 时仍尝试修复一次（不做第2次复核以节省 token）。
    ...
    for (let bi = 0; bi < qaResult.criticalBugs.length; bi++) {
      ...
      const fixed = await agentR(
        [
          ...
          '修复后确认 ' + EP.verifyCmd + ' 以 exit 0 结束，限定路径 add 后提交: `git add game state ' + DOCS.techStack + ' && git commit -m "phase2: fix QA — ' + bug.title + '"`（禁止 `git add -A`、禁止整个指定 `.codex/docs` 目录。' + DOCS.techStack + ' 是为了把下述陷阱提升记录包含在同一提交中 — 仅在追加写入时才会被 stage）。',
        ...
        { label: 'fix-qa-r' + round + '-' + bug.assignee + '-' + bi, phase: 'QA', agentType: bug.assignee, effort: 'medium' }
      );
      if (fixed === null) {
    ...
    if ((qaResult.failedAcceptance || []).length > 0) {
      const faFixed = await agentR(
        [
          ...
          '修复后确认 ' + EP.verifyCmd + ' 以 exit 0 结束，限定路径 add 后提交: `git add game state ' + DOCS.techStack + ' && git commit -m "phase2: fix QA — failed acceptance"`（禁止 `git add -A`、禁止整个指定 `.codex/docs` 目录。' + DOCS.techStack + ' 是为了把陷阱提升记录包含在同一提交中）。',
          '若修复原因是引擎/测试运行器导致的一般性问题（环境陷阱），立即追加写入 tech-stack 文档的「已知陷阱」节（没有则新建 — gates.md QA-PLAY）。',
          IDEMPOTENT_RULE,
          '简洁返回修复内容。',
        ].join('\n'),
        { label: 'fix-qa-acceptance-r' + round, phase: 'QA', agentType: 'gameplay-engineer', effort: 'medium' }
      );
      if (faFixed === null) {
