...
1. **命名、ID、路径遵循 contract** — 禁止自创。 @.claude/docs/contract.md
2. **所有产出物都要经过 produce→review→revise 循环** — 合格标准见 @.claude/docs/review-loops.md
3. **游戏实现遵循按引擎区分的 tech-stack 规范** — phaser: @.claude/docs/tech-stack.md / unity: `.claude/docs/tech-stack-unity.md` / unreal: `.claude/docs/tech-stack-unreal.md`（engine 见 `state/engine.txt`。不存在则为 phaser）
4. **资产生成遵循路由表** — @.claude/docs/assets-config.md
5. **状态以文件为事实** — 读取 `state/` 而不是对话。工作后更新 `state/active.md`
6. **支柱（P-xx）是北极星** — 所有设计、实现、QA 判断都对照 `design/concept.md` 的支柱

## 流水线全貌

@.claude/docs/pipeline.yaml 定义各阶段。Gate 判定提示词见 @.claude/docs/gates.md 。

## 本仓库的结构

- `.claude/` — harness 本体（agents / skills / workflows / hooks / rules / docs / tests）
